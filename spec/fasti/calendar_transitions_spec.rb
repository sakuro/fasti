...
RSpec.describe Fasti::CalendarTransitions do
  describe ".gregorian_start_jdn" do
    it "returns correct JDN for Italy" do
      expect(described_class.gregorian_start_jdn(:it)).to eq(Date::ITALY)
    end

    it "returns correct JDN for Great Britain" do
      expect(described_class.gregorian_start_jdn(:gb)).to eq(Date::ENGLAND)
    end

    it "returns correct JDN for United States (follows British)" do
      expect(described_class.gregorian_start_jdn(:us)).to eq(Date::ENGLAND)
    end

    it "returns correct JDN for Russia" do
      expect(described_class.gregorian_start_jdn(:ru)).to eq(2_421_639)
    end

    it "returns default (Italian) JDN for unknown countries" do
      expect(described_class.gregorian_start_jdn(:unknown)).to eq(Date::ITALY)
    end
  ...
  describe ".create_date" do
    context "with Italian transition (1582)" do
      it "creates Julian dates before transition" do
        date = described_class.create_date(1582, 10, 4, :it)
      ...
      it "creates Gregorian dates after transition" do
        date = described_class.create_date(1582, 10, 15, :it)
        expect(date.year).to eq(1582)
        expect(date.month).to eq(10)
      ...
      it "raises error for dates in the gap" do
        expect do
          described_class.create_date(1582, 10, 10, :it)
        end.to raise_error(ArgumentError, /does not exist in IT due to calendar transition/)
      end
    end

    context "with British transition (1752)" do
      it "creates Julian dates before transition" do
        date = described_class.create_date(1752, 9, 2, :gb)
      ...
      it "creates Gregorian dates after transition" do
        date = described_class.create_date(1752, 9, 14, :gb)
        expect(date.year).to eq(1752)
        expect(date.month).to eq(9)
      ...
      it "raises error for dates in the British gap" do
        expect do
          described_class.create_date(1752, 9, 10, :gb)
        end.to raise_error(ArgumentError, /does not exist in GB due to calendar transition/)
      end
    end

    context "with modern dates" do
      it "creates modern dates normally" do
        date = described_class.create_date(2024, 9, 3, :jp)
  ...
  describe ".valid_date?" do
    ...
    it "returns false for dates in British transition gap" do
      ...
      expect(described_class.valid_date?(british_gap_date, :gb)).to be false
    end

    it "returns true for valid modern dates" do
      expect(described_class.valid_date?(valid_date, :jp)).to be true
    end

    it "returns true for dates before transition" do
      before_british = Date.new(1752, 9, 2)
      expect(described_class.valid_date?(before_british, :gb)).to be true
    end

    it "returns true for dates after transition" do
      after_british = Date.new(1752, 9, 14)
      expect(described_class.valid_date?(after_british, :gb)).to be true
    end
  end

  describe ".transition_info" do
    it "returns correct information for Great Britain" do
      info = described_class.transition_info(:gb)
    ...
    it "returns correct information for Italy" do
      info = described_class.transition_info(:it)
    ...
    it "returns correct information for Russia" do
      info = described_class.transition_info(:ru)
